...
class Scanner():
  ...
  def classifyColour(self, pixel):
    ...
    '''
    # print(pixel)
    colour_vals = [[121, 0,  0], [0, 119, 0], [0, 0, 118], [0, 100, 100]]
    colour_names = ["blue", "green", "red", "yellow"]
    colour_dists = []
  ...
  def scanBuoy(self, img):
    ...
    xx,yy =binary.shape
    mask = mask[int(0*xx/3):int(1*xx/2),:]
    ...
    if cv2.contourArea(best_cnt) < 200 or cv2.contourArea(best_cnt) > 1000:
        return "none"
