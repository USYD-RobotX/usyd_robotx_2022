...
class DockingSolution:
    ...
    #     linear: 
    #   x: 0.14780634833924133
    #   y: -0.05019779000284538
    #   z: -0.006197084294320434
    # angular: 
    #   x: 0.000824380724064129
    #   y: -0.010360770888624209
    #   z: -0.005225817467643186


        wp1 = Pose() # entrance
        wp1.position.x = 216.71614841823003
        wp1.position.y = 152.60702980068012
        wp1.position.z = -0.0025008686253636856
        wp1.orientation.x = -0.00021305488303824264
        wp1.orientation.y = 0.008259410483930758
        wp1.orientation.z = 0.4543721192012582
        wp1.orientation.w = 0.890773660386651

        wp2 = self.translatePose(wp1, 0, 10, 0)

        wp3 = Pose()
        wp3.position.x= 229.5974163375557
        wp3.position.y= 175.85892895334368
        wp3.position.z= -0.0013672306272796394
        wp3.orientation.x= -0.0016108224776182323
        wp3.orientation.y= 0.007911579800763978
        wp3.orientation.z= 0.5278171950906738
        wp3.orientation.w= 0.8493196222404233


        wp4 = self.translatePose(wp2, 0, 0, 3.1415)

        wp5 = self.translatePose(wp1, 0, 0, 3.1415)

        self.go_to_pose(wp1, 10, 0.7)

        self.go_to_pose(wp2, 10, 0.4)

        self.go_to_pose(wp3, 5, 0.3)
        self.go_to_pose(wp4, 1, 0.3)
        self.go_to_pose(wp5, 2, 0.3)
