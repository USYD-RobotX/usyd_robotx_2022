...
class lightScanner():
  ...
  def scanBuoy(self, img):
    ...
    lower_colour = np.array([0, 30, 254])
    upper_colour = np.array([360, 255, 255])
    ...
    blur= cv2.GaussianBlur(src=blur, ksize=(5, 5), sigmaX=0)
    ...
    xx,yy =binary.shape
    blur= blur[int(0*xx/3):int(1*xx/2),:]
    img = img[int(0*xx/3):int(1*xx/2),:,:]
    # cv2.imshow("binary", mask)
    # cv2.waitKey(1)

    # Find contours of image
    cont_return = cv2.findContours(blur, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    contours = cont_return[0] if len(cont_return) == 2 else cont_return[1] # Version fix
    ...
    if cv2.contourArea(best_cnt) < 200 or cv2.contourArea(best_cnt) > 10000:
        return "none2"
