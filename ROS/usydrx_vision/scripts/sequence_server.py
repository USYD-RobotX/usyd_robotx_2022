...
class Cam(object):
    ...
    def capture(self):
        while True:
            ...
            ros_img = rospy.wait_for_message("/wamv/sensors/cameras/front_camera/image_raw", Image)
...
class SequenceFinder(object):
    ...
    def colorFinder(self):
        ...
        if self.sample:
            ...
            a = ['red','blue','green','red','blue','green','red','blue','green','red','blue','green']
            # get the image
            # CHANGE THIS TO READ THE MIDDLE CAMERA FOR THE LIVE DETECTION
            rospack = rospkg.RosPack()
            img = cv2.imread(rospack.get_path('usydrx_vision')+'/Images/seq_'+a[self.idx]+'.png',cv2.IMREAD_COLOR)
            self.idx += 1
        ...
        print("save_image")
        cv2.imwrite("test.png", self.capturedImg)
        os.exit()
