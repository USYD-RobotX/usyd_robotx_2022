"""LCM type definitions
This file automatically generated by lcm.
DO NOT MODIFY BY HAND!!!!
"""

try:
    import cStringIO.StringIO as BytesIO
except ImportError:
    from io import BytesIO
import struct

class IMU_t(object):
    __slots__ = ["utime", "angRate", "accel", "deltaAngle", "deltaVelocity"]

    def __init__(self):
        self.utime = 0
        self.angRate = [ 0.0 for dim0 in range(3) ]
        self.accel = [ 0.0 for dim0 in range(3) ]
        self.deltaAngle = [ 0.0 for dim0 in range(3) ]
        self.deltaVelocity = [ 0.0 for dim0 in range(3) ]

    def encode(self):
        buf = BytesIO()
        buf.write(IMU_t._get_packed_fingerprint())
        self._encode_one(buf)
        return buf.getvalue()

    def _encode_one(self, buf):
        buf.write(struct.pack(">q", self.utime))
        buf.write(struct.pack('>3d', *self.angRate[:3]))
        buf.write(struct.pack('>3d', *self.accel[:3]))
        buf.write(struct.pack('>3d', *self.deltaAngle[:3]))
        buf.write(struct.pack('>3d', *self.deltaVelocity[:3]))

    def decode(data):
        if hasattr(data, 'read'):
            buf = data
        else:
            buf = BytesIO(data)
        if buf.read(8) != IMU_t._get_packed_fingerprint():
            raise ValueError("Decode error")
        return IMU_t._decode_one(buf)
    decode = staticmethod(decode)

    def _decode_one(buf):
        self = IMU_t()
        self.utime = struct.unpack(">q", buf.read(8))[0]
        self.angRate = struct.unpack('>3d', buf.read(24))
        self.accel = struct.unpack('>3d', buf.read(24))
        self.deltaAngle = struct.unpack('>3d', buf.read(24))
        self.deltaVelocity = struct.unpack('>3d', buf.read(24))
        return self
    _decode_one = staticmethod(_decode_one)

    _hash = None
    def _get_hash_recursive(parents):
        if IMU_t in parents: return 0
        tmphash = (0x5ac3c119a56a65cf) & 0xffffffffffffffff
        tmphash  = (((tmphash<<1)&0xffffffffffffffff)  + (tmphash>>63)) & 0xffffffffffffffff
        return tmphash
    _get_hash_recursive = staticmethod(_get_hash_recursive)
    _packed_fingerprint = None

    def _get_packed_fingerprint():
        if IMU_t._packed_fingerprint is None:
            IMU_t._packed_fingerprint = struct.pack(">Q", IMU_t._get_hash_recursive([]))
        return IMU_t._packed_fingerprint
    _get_packed_fingerprint = staticmethod(_get_packed_fingerprint)

