...
class gpsd3_orig_t(object):
    __slots__ = ["utime", "online", "fix", "geoidal_separation", "status", "satellites_used", "used", "dop", "epe", "skyview_utime", "satellites_visible", "PRN", "elevation", "azimuth", "ss", "dev", "tag"]
    ...
    def encode(self):
        buf = BytesIO()
        buf.write(gpsd3_orig_t._get_packed_fingerprint())
    ...
    def decode(data):
        if hasattr(data, 'read'):
            ...
            buf = BytesIO(data)
        if buf.read(8) != gpsd3_orig_t._get_packed_fingerprint():
            raise ValueError("Decode error")
        return gpsd3_orig_t._decode_one(buf)
    ...
    def _get_hash_recursive(parents):
        if gpsd3_orig_t in parents: return 0
        newparents = parents + [gpsd3_orig_t]
        tmphash = (0x5fd46a69f78ecfe4+ senlcm.gpsd3_fix_t._get_hash_recursive(newparents)+ senlcm.gpsd3_dop_t._get_hash_recursive(newparents)+ senlcm.gpsd3_devconfig_t._get_hash_recursive(newparents)) & 0xffffffffffffffff
    ...
    def _get_packed_fingerprint():
        if gpsd3_orig_t._packed_fingerprint is None:
            gpsd3_orig_t._packed_fingerprint = struct.pack(">Q", gpsd3_orig_t._get_hash_recursive([]))
        return gpsd3_orig_t._packed_fingerprint
